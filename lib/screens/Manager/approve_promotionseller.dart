...
import 'package:k6_app/screens/Manager/approve_promotion_detail.dart';
import 'package:k6_app/utility/my_constant.dart';
import 'package:k6_app/utility/my_style.dart';

class ApprovePromotion extends StatefulWidget {
  @override
  _ApprovePromotionState createState() => _ApprovePromotionState();
}

class _ApprovePromotionState extends State<ApprovePromotion> {
    ...
    await Dio().get(url).then((value) {
      ...
      if (value.toString() != 'null') {
        ...
        for (var map in result) {
          ...
          setState(() {
            if (promotionsellerModel.status == 'no') {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: Center(child: Text('อนุมัติโปรโมชั่น')),
        ),
                ...
                child: ListTile(
                  onTap: () {
                    MaterialPageRoute route = MaterialPageRoute(
                        builder: (value) => DetailpromotionAprove(
